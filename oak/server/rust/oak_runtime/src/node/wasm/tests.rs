...
fn start_node(log: Logger, wasm_module: Vec<u8>, entrypoint_name: &str) -> Result<(), OakStatus> {
    ...
    let proxy =
        RuntimeProxy::create_runtime(log, application_configuration, GrpcConfiguration::default());
